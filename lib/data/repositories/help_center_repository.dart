...
class HelpCenterRepository {
  ...
  Future<Result<List<FaqModel>>> getFaq() async {
    final result = await _client.get<List<dynamic>>('/about/frequently_asked_questions/list/');
    return result.fold(
          (error) => Result.error(error),
          (value) => Result.ok(
        value.map((e) => FaqModel.fromJson(e)).toList(),
      ),
    );
  }
}
