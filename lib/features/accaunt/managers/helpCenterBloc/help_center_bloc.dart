  ...
  Future<void> _onContentLoading(
    ...
    result.fold(
        ...
        state.copyWith(status: Status.error, errorMessage: error.toString()),
          ...
          (value) => emit(
        state.copyWith(status: Status.success, contacts: value),
      ),
    );
  }

  Future<void> _onFaqLoading(
      FaqLoading event,
      Emitter<HelpCenterState> emit,
      ) async {
    emit(state.copyWith(status: Status.loading));
    final result = await _helpCenterRepo.getFaq();
    ...
    result.fold(
        ...
        state.copyWith(status: Status.error, errorMessage: error.toString()),
          ...
          (value) => emit(
        state.copyWith(status: Status.success, faqs: value),
      ),
    );
  }
}
